...
class MulticlassingArticlePage {
  ...
  factory MulticlassingArticlePage.fromJson(JsonObject json) {
    ...
    return MulticlassingArticlePage(
      ...
      proficiencies: [
        for (var it in json['proficiencies']) DndRef.fromJson(it)
      ],
  ...
  List<Widget> build() {
    ...
    if (prerequisites != null) {
      ...
      prerequisiteOptions!.forEach((key, value) => children.add(ListTileRef(
            ref: key,
            trailing: Text(
              value.toString(),
              style: const TextStyle(fontSize: 16.0),
    ...
    if (proficiencyChoices != null) {
      children += [
        ...
        for (var choice in proficiencyChoices!)
          annotatedLine(
            annotation:
                "Choose ${choice['choose'].toString()} ${choice['desc']}: ",
            contents: [
